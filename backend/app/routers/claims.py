...
def _sync_claim_to_blockchain(db: Session, claim: Claim, current_user) -> dict:
    ...
    if claim.blockchain_claim_id:
        return {
            "claim_id": claim.blockchain_claim_id,
            "submit_tx_hash": claim.submit_tx_hash,
            "blockchain_status": claim.blockchain_status,
        }

    amount = float(get_payout_amount(claim.event_type, claim.policy.tier))
    submitted = web3_gateway.submit_claim_for(
    ...
    return {
        ...
        "submit_tx_hash": submitted["tx_hash"],
        "verify_tx_hash": verified["verify_tx_hash"],
        "authorize_tx_hash": verified["authorize_tx_hash"],
        "blockchain_status": "AUTHORIZED",
    }
...
def submit_claim(payload: ClaimCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if claim.verification_status == "VALID":
        try:
            ...
            response.update({
                "message": "Claim verified by ML, but blockchain authorization is pending.",
    ...
    response = claim_dict(claim)
    response.update({
        "message": "Claim submitted, ML verified, blockchain authorized, and payout processing started." if claim.blockchain_status == "AUTHORIZED" else "Claim submitted and verification completed.",
        "verification": verification,
