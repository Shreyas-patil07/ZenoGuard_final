...
def get_active_locked_policy(db: Session, rider_id: int) -> Optional[Policy]:
    ...
    return (db.query(Policy).filter(Policy.rider_id == rider_id, Policy.locked == True, Policy.active == True, Policy.end_date >= now).order_by(Policy.id.desc()).first())
...
def _policy_dict(p: Policy) -> dict:
    ...
    return {
        "policy_id": p.id, "tier": p.tier, "tier_label": tier_info["label"], "duration_days": p.duration_days,
        "premium": p.premium, "total_premium": round(p.premium * p.duration_days, 2), "risk_score": p.risk_score,
        "active": p.active, "locked": p.locked,
...
def get_active_policy(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    return {"has_active_policy": bool(policy), "policy": _policy_dict(policy) if policy else None}
...
def activate_policy(payload: PremiumRequest, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    return {"message": f"Policy activated and locked for {duration} days.", "policy": _policy_dict(policy)}
