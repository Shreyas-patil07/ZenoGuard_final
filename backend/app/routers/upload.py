...
async def upload_kyc_document(
    ...
    if document_type != "driving_license":
        raise HTTPException(status_code=422, detail="Only Driving Licence verification is currently enabled.")

    result = await _upload(file, f"zenoguard/kyc/{current_user.id}/driving_license")
    ...
    profile.ai_verification_note = "Saved. Driving licence AI verification runs only when KYC is submitted."
    ...
    return {
        ...
        "uploaded": True,
        "verification_status": "pending_submit",
        **result,
        "message": "Driving Licence saved. AI verification will run when you submit KYC.",
    }
...
async def upload_kyc_secondary(file: UploadFile = File(...), current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    raise HTTPException(status_code=410, detail="Secondary identity documents are temporarily disabled.")
