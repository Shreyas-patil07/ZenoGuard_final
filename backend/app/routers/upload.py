...
async def upload_kyc_document(file: UploadFile = File(...), current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    _, _, quality_issues = _quality(image)
    if quality_issues:
    ...
    if verification["status"] != "verified":
        ...
        return {
            ...
            "message": "Driving licence needs review. Upload a clearer, unobstructed driving licence image.",
        }

    profile.id_type = "driving_license"
    profile.id_number = verification["fields"].get("dl_number") or profile.id_number
    profile.date_of_birth = verification["fields"].get("dob") or profile.date_of_birth
    ...
    if not _cloudinary_ready():
        raise HTTPException(status_code=503, detail="Cloudinary is not configured on the backend.")
    try:
        result = cloudinary.uploader.upload(io.BytesIO(raw), folder=f"zenoguard/kyc/{current_user.id}", resource_type="image", overwrite=False)
    except Exception as exc:
        db.rollback()
        raise HTTPException(status_code=502, detail="Cloudinary upload failed.") from exc

    profile.id_document_url = result.get("secure_url", "")
    db.commit()
    db.refresh(profile)
    return {"document_type": "driving_license", "uploaded": True, "cloudinary_url": profile.id_document_url, "verification": verification, "message": "Driving licence passed the automated document AI checks."}
...
async def upload_kyc_secondary(file: UploadFile = File(...), current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if secondary_type not in ALLOWED_SECONDARY:
        raise HTTPException(status_code=422, detail="Choose Aadhaar or PAN in the profile before uploading the additional document.")

    result = await _upload(file, f"zenoguard/kyc/{current_user.id}")
    profile.secondary_id_document_url = result["cloudinary_url"]
    db.commit()
    db.refresh(profile)
    return {"document_type": secondary_type, "uploaded": True, **result}
