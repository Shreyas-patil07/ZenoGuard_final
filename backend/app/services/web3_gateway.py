...
def build_driver_registration(wallet: str, driver_id: str) -> dict[str, Any]:
    ...
    tx = contract.functions.registerDriver(driver_hash).build_transaction({
        "from": wallet,
...
def build_policy_purchase(
    ...
    tx = contract.functions.purchasePolicy(
        premium_wei,
        coverage_wei,
...
def get_onchain_policy(policy_id: int) -> dict[str, Any]:
    ...
    return {
        "id": int(policy[0]),
        "driver": policy[1],
        "premium_wei": int(policy[2]),
        "coverage_wei": int(policy[3]),
        "start_time": int(policy[4]),
        "expiry_time": int(policy[5]),
        "active": bool(policy[6]),
        "under_review": bool(policy[7]),
    }
...
def get_onchain_claim(claim_id: int) -> dict[str, Any]:
    ...
    return {
        "id": int(claim[0]),
        "policy_id": int(claim[1]),
        "amount_wei": int(claim[2]),
        "submitted": bool(claim[3]),
        "accident_verified": bool(claim[4]),
        "approved": bool(claim[5]),
        "paid": bool(claim[6]),
    }
...
def _serialise_tx(tx: dict[str, Any]) -> dict[str, Any]:
    return {
        key: (Web3.to_hex(value) if isinstance(value, (bytes, bytearray)) else int(value) if isinstance(value, int) else value)
        for key, value in tx.items()
        if key in {"to", "from", "data", "value", "gas", "maxFeePerGas", "maxPriorityFeePerGas", "gasPrice", "nonce", "chainId"}
    }
