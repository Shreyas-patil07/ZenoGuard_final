...
def _request(method: str, path: str, *, json_body: Optional[dict] = None, headers: Optional[dict] = None) -> dict:
...
def create_order(*, amount_inr: float, receipt: str, notes: Optional[dict] = None) -> dict:
    ...
    return _request("POST", "/orders", json_body={
        "amount": amount_paise,
        "currency": "INR",
        "receipt": receipt,
        "notes": notes or {},
...
def create_contact(*, name: str, email: str, phone: str, reference_id: str) -> dict:
    return _request("POST", "/contacts", json_body={
        "name": name[:50],
        "email": email,
        "contact": phone,
        "type": "customer",
...
def create_vpa_fund_account(*, contact_id: str, upi_id: str) -> dict:
    return _request("POST", "/fund_accounts", json_body={
