...
export default function WalletPayments({ Shell }) {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        const checkout = new window.Razorpay({
          ...
          amount: order.amount_paise ?? Math.round(Number(order.amount_inr || 0) * 100),
  ...
  if (loading) return <Shell title="Protection & payments"><div className="content-narrow"><div className="panel"><Loader2 className="spin"/> Loading protection and wallet...</div></div></Shell>;
  ...
  return <Shell title="Protection & payments">
    <div className="dashboard-grid">
      <div className="metric-card featured"><div className="metric-label"><ShieldCheck size={17}/> POLICY STATUS</div><strong>{active ? 'ACTIVE' : pending ? 'PAYMENT PENDING' : 'NOT ACTIVE'}</strong><span>{active ? `${active.tier_label} · ${active.days_remaining ?? 0} days remaining` : 'Premium payment activates coverage.'}</span><div className="metric-bottom"><span>{active ? `Coverage up to ₹${Number(active.coverage?.accident || 0).toLocaleString('en-IN')}` : 'Choose a plan below'}</span><button onClick={load}><RefreshCw size={15}/></button></div></div>
      <div className="metric-card"><div className="metric-label"><IndianRupee size={17}/> WALLET BALANCE</div><h3>₹{Number(wallet?.balance || 0).toFixed(2)}</h3><span>{wallet?.payout_method || 'No payout method configured'}</span></div>
      <div className="metric-card"><div className="metric-label"><WalletCards size={17}/> SETTLEMENT</div><h3>{wallet?.payout_method ? 'UPI payout ready' : 'Configure UPI'}</h3><span>Claim payouts can be routed through the Razorpay payout account.</span></div>
    </div>
    ...
    {active && <div className="panel"><div className="eyebrow">ACTIVE POLICY</div><h2>{active.tier_label} protection</h2><div className="info-row"><ShieldCheck size={20}/><div><b>Active until {new Date(active.end_date).toLocaleDateString('en-IN')}</b><span>₹{Number(active.total_premium).toFixed(2)} paid · {active.duration_days} days</span></div></div><div className="platform-grid"><div className="platform-card selected"><b>Accident</b><span>₹{Number(active.coverage?.accident || 0).toLocaleString('en-IN')}</span></div><div className="platform-card selected"><b>Breakdown</b><span>₹{Number(active.coverage?.breakdown || 0).toLocaleString('en-IN')}</span></div><div className="platform-card selected"><b>Weather</b><span>₹{Number(active.coverage?.weather || 0).toLocaleString('en-IN')}</span></div></div></div>}

    <div className="split-grid"><div className="panel"><div className="eyebrow">PAYOUT METHOD</div><h2>Where should claim payouts go?</h2><form className="form-stack" onSubmit={savePayoutMethod}><label>UPI ID<input required placeholder="name@upi" value={upi} onChange={e=>setUpi(e.target.value)}/></label><label>Phone number<input required placeholder="9876543210" value={phone} onChange={e=>setPhone(e.target.value)}/></label><button className="primary-btn submit" disabled={busy === 'payout'}>{busy === 'payout' ? 'Saving...' : 'Save UPI payout method'} <ArrowRight size={17}/></button></form></div><div className="panel"><div className="eyebrow">WALLET</div><h2>Claim payout balance</h2><p className="muted">Approved claim payouts are credited here. You can withdraw to the configured UPI account.</p><button className="primary-btn submit" disabled={!wallet?.balance || !wallet?.payout_method || busy === 'withdraw'} onClick={withdraw}>{busy === 'withdraw' ? 'Processing...' : 'Withdraw to UPI'} <ArrowRight size={17}/></button></div></div>

    <div className="panel"><div className="eyebrow">RECENT TRANSACTIONS</div>{wallet?.transactions?.length ? wallet.transactions.map(tx=><div className="info-row" key={tx.id}><IndianRupee size={18}/><div><b>{tx.description}</b><span>{tx.status} · {tx.timestamp ? new Date(tx.timestamp).toLocaleString('en-IN') : ''}</span></div><strong>{tx.amount >= 0 ? '+' : ''}₹{Number(tx.amount).toFixed(2)}</strong></div>) : <p className="muted">No wallet transactions yet.</p>}</div>
  </Shell>;
}
